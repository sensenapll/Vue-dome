...
export default {
  ...
  // 根据分数来计算有几个on,有几个half,有几个off---最终肯定一共有五个,需要用一个容器把on,half,off存储起来
  // 分数决定着数组中的on,half,off的个数
  ...
  computed: {
    classArr() {
      ...
      // 先获取分数
      const { score } = this
      // 4.7----- 4个满星   07>=0.5 ---半颗星星(1个)
      // 3.6-----3个满星   0.6>=0.5---半颗星星(1个)----1个灰星
      // 3.1----3个满星  01<=0.5 -----0个半颗星星---2个灰星
      // 取整的分数
